...
public class BookService(ApplicationDbContext dbContext) : IBookService
{
    ...
    public async Task<PaginatedBook<Models.Book>> GetSearchedBook(string searchString, int page = 1, int pageSize = 6)
    {
        ...
        var totalBooks = await _context.Books.Where(b => b.Category != null && (b.BookName.Contains(searchString)
            || b.Author.Contains(searchString) 
            || b.Isbn.Contains(searchString)
            ||(b.Category.CategoryName.Contains(searchString))
            || b.Publisher.Contains(searchString))).CountAsync();
        
        var books = await _context.Books.Where(b => b.Category != null && (b.BookName.Contains(searchString)
                                                                           || b.Author.Contains(searchString)
                                                                           || b.Isbn.Contains(searchString)
                                                                           || b.Category.CategoryName.Contains(searchString)
    ...
    public async Task<bool> DecreaseBookQuantity(int id)
    {
        if (id < 1)
            return false;
        var book = await _context.Books.FindAsync(id);
        ...
        if (book.Quantity < 1)
             throw new NotEnoughException(book.BookName); 
        book.Quantity--;
        _context.Books.Update(book);
